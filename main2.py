...
class AutoModeThread(QThread):
    ...
    def automa(self):
        while self.vehicle.mode != VehicleMode('AUTO'):
            print("Waiting for vehicle to Auto mode")
...
class VehicleSpeedThread(QThread):
    ...
    def run(self):
        while True:  # Thread sürekli olarak çalışacak
            # İHA'dan hız verisini al
            speed = self.getVehicleSpeed()  # İHA'dan hız verisini al
            self.speedChanged.emit(speed)  # Hız verisini sinyal olarak gönder
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # >>>>>>>>>>>>>>>>>>>>>İha Verileri Bölümü<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        # Speed değişkeninin threadini güncelleme
    ...
    def veriyial(self):
        self.data_thread = VehicleSpeedThread(self.vehicle)
        self.data_thread.speedChanged.connect(self.updateSpeedDisplay)
